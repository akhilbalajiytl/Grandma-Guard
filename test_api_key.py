...
try:
    ...
    if response.status_code == 200:
        ...
        print(
            "   This means the API key is INVALID, EXPIRED, or has INSUFFICIENT QUOTA."
        )
        print(
            "   The key itself is the problem. Please ask your supervisor to check the key's status in the OpenAI dashboard."
        )
        ...
        print(
            "   This could be 'requests per minute' or 'tokens per minute'. Wait a minute and try again."
        )
        print("   Raw Error:", response.text)
    else:
